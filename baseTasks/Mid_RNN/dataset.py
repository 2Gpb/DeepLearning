...
class TextDataset(Dataset):
    ...
    def __init__(self, data_file: str, train: bool = True, sp_model_prefix: str = None,
        ...
        rng = np.random.default_rng(self.TRAIN_VAL_RANDOM_SEED)
        indices = rng.permutation(len(texts))
        end_train_i = int(len(indices) * (1 - self.VAL_RATIO))
        train_texts = [texts[i] for i in indices[:end_train_i]]
        val_texts = [texts[i] for i in indices[end_train_i:]]
    ...
    def __getitem__(self, item: int) -> Tuple[torch.Tensor, int]:
        ...
        """
        cur_indices = self.indices[item][:self.max_length - 2]
        length = len(cur_indices) + 2

        indices = torch.full((self.max_length,), self.pad_id, dtype=torch.long)
        indices[0] = self.bos_id
        indices[1:length - 1] = torch.tensor(cur_indices, dtype=torch.long)
